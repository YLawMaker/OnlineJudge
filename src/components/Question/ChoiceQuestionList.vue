<template>
  <el-card>
    ...
    <el-row type="flex" justify="space-between">
      <el-col :span="3">
        <el-button type="default" @click="handleAdd()" round
          >添加选择题</el-button
        >
      </el-col>
      <el-col :span="21">
        ...
        <el-table-column label="操作" width="300px" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              type="danger"
              size="mini"
              @click="deleteCohiceQuestionInfo(scope.row.choiceQuestionId)"
    ...
    >
      <el-form
        :model="choiceQuestionInfo"
        ref="choiceQuestionInfo"
        label-width="80px"
        label-position="right"
        :rules="submitRules"
      >
        <el-form-item prop="choiceQuestionDescription" label="题目描述">
          <el-input
            v-model="choiceQuestionInfo.choiceQuestionDescription"
            placeholder="题目描述"
            type="textarea"
            :autosize="true"
        ...
        <el-form-item label="考题选项" :required="true">
          <el-table
            :data="choice"
            style="width: 100%"
            :cell-style="rowClass"
          ...
          >
            <el-table-column prop="name" width="100px" label="选项">
            </el-table-column>
            <el-table-column prop="input" label="选项内容">
              <template slot-scope="scope">
                <!-- 选项内容的错误红色框 -->
                <el-input
                  v-model="scope.row.input"
                  placeholder="请输入内容"
                  :class="{ w1: isChoiceInputErrorVisible }"
                >
                </el-input>
              </template>
            </el-table-column>
            <el-table-column prop="choiceId" label="答案" width="100px">
              <template slot-scope="scope">
                <!-- 选项正确选项的错误红色框 -->
                <el-radio
                  v-model="radio"
                  :label="scope.row.choiceId"
                  :class="{ w2: isChoiceErrorVisible }"
                >
                  <span></span>
                </el-radio>
              </template>
        ...
        <el-form-item label="标签" prop="questionLabels">
          ...
          <el-select
            v-model="choiceQuestionInfo.questionLabels"
            multiple
            placeholder="请选择"
    ...
    >
      <el-form
        :model="eChoiceQuestionInfo"
        ref="eChoiceQuestionInfo"
        label-width="80px"
        label-position="right"
        :rules="editSubmitRules"
      >
        <el-form-item prop="choiceQuestionDescription" label="题目描述">
          <el-input
            v-model="eChoiceQuestionInfo.choiceQuestionDescription"
            placeholder="题目描述"
            type="textarea"
            :autosize="true"
            style="resize: none; font-size: 1.2em"
        ...
        >
          <el-select
            v-model="eChoiceQuestionInfo.choiceQuestionDifficulty"
            placeholder="请选择"
          >
            <el-option
              v-for="item in difficultyOption"
    ...
    <div class="e1">
      ...
      >
        <el-form
          class="choiceQuestionShowForm"
          :model="showChoiceQuestionInfo"
          ref="showChoiceQuestionInfo"
          label-width="100px"
          label-position="right"
        >
          <el-form-item prop="choiceQuestionDescription" label="题目描述">
            <el-input
              v-model="showChoiceQuestionInfo.choiceQuestionDescription"
              type="textarea"
              :autosize="true"
              :disabled="true"
            >
          ...
          <el-form-item prop="choiceQuestionStemA" label="题目A选项">
            <el-input
              v-model="showChoiceQuestionInfo.choiceQuestionStemA"
              type="textarea"
              :autosize="true"
              :disabled="true"
            >
          ...
          <el-form-item prop="choiceQuestionStemB" label="题目B选项">
            <el-input
              v-model="showChoiceQuestionInfo.choiceQuestionStemB"
              type="textarea"
              :autosize="true"
              :disabled="true"
            >
          ...
          <el-form-item prop="choiceQuestionStemC" label="题目C选项">
            <el-input
              v-model="showChoiceQuestionInfo.choiceQuestionStemC"
              type="textarea"
              :autosize="true"
              :disabled="true"
            >
          ...
          <el-form-item prop="choiceQuestionStemD" label="题目D选项">
            <el-input
              v-model="showChoiceQuestionInfo.choiceQuestionStemD"
              type="textarea"
              :autosize="true"
              :disabled="true"
            >
          ...
          <el-form-item prop="chapter" label="所属章节">
            <!-- 初始化页面时不让他初始化没有对应的值 -->
            <el-input
              v-model="showChoiceQuestionInfo.questionLabels[0].chapter"
              type="textarea"
              :autosize="true"
              :disabled="true"
              v-if="showChoiceQuestionInfo.questionLabels.length > 0"
            >
            </el-input>
          </el-form-item>
          <el-form-item prop="firstKnowledgePoint" label="第一知识点">
            <el-input
              v-model="
                showChoiceQuestionInfo.questionLabels[0].firstKnowledgePoint
              "
              placeholder="第一知识点"
              type="textarea"
              :autosize="true"
          ...
          <el-form-item prop="secondKnowledgePoint" label="第二知识点">
            <el-tag
              type="info"
              effect="dark"
              v-for="(item, index) in showChoiceQuestionInfo.questionLabels"
              :key="index"
            >
              {{ item.secondKnowledgePoint }}
            </el-tag>
          ...
          >
            <el-form-item label="是否私有">
              <el-input
                v-model="showChoiceQuestionInfo.isPrivate"
                type="textarea"
                :disabled="true"
              >
              </el-input>
            ...
            <el-form-item label="题目难度">
              <el-input
                v-model="showChoiceQuestionInfo.choiceQuestionDifficulty"
                type="textarea"
                :disabled="true"
              >
              </el-input>
            </el-form-item>
            <el-form-item label="题目正确选项">
              <el-input
                v-model="showChoiceQuestionInfo.choiceQuestionCorrectOption"
                type="textarea"
                :disabled="true"
              >
              </el-input>
        ...
        <!-- 不需要按钮 -->
        <!-- <div slot="footer" class="dialog-footer">
          <el-button style="width" @click="choiceQuestionShowVisible = false">确定</el-button>
        </div> -->
      </el-dialog>
    </div>
    ...
    <!-- <el-dialog
      title="删除选择题"
      :visible.sync="deleteChoiceQuestionVisible"
      center
      width="400px"
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="deleteChoiceQuestionVisible = false"
          >取 消</el-button
        >
  ...
  methods: {
    ...
    addFirstKnowledgePointClick () {
      ...
      this.getSecondKnowledgePointInfo(
        this.chapterOptions[this.chapterChoice].label,
        this.firstKnowledgePointOptions[this.firstKnowledgePointChoice].label
      );
    },
    //修改信息点击章节
    editChapterClick () {
      ...
      this.$refs[choiceQuestionInfo].validate((valid) => {
        ...
        if (valid) {
          ...
          if (
            ...
            this.choiceQuestionInfo.choiceQuestionCorrectOption = this.choice[
              this.radio - 1
            ].name;
            for (
              var i = 0;
              i < this.choiceQuestionInfo.questionLabels.length;
              i++
            ) {
              var questionLabel = new Object();
              questionLabel.questionLabelId = this.choiceQuestionInfo.questionLabels[
                i
              ];
      ...
      this.$refs[choiceQuestionInfo].validate((valid) => {
        ...
        if (valid) {
          ...
          if (
            ...
            this.eChoiceQuestionInfo.choiceQuestionCorrectOption = this.choice[
              this.radio - 1
            ].name;
            for (
              var i = 0;
              i < this.eChoiceQuestionInfo.questionLabels.length;
              i++
            ) {
              var questionLabel = new Object();
              questionLabel.questionLabelId = this.eChoiceQuestionInfo.questionLabels[
                i
              ];
    ...
    handleShow (row) {
      ...
      this.showChoiceQuestionInfo.choiceQuestionCorrectOption =
        row.choiceQuestionCorrectOption;
      this.showChoiceQuestionInfo.choiceQuestionDifficulty =
        row.choiceQuestionDifficulty;
      ...
      this.$axios({
        ...
        .then((res) => {
          ...
          for (var i = 0; i < this.secondKnowledgePointList.length; i++) {
            ...
            secondKnowledgePoint.label = this.secondKnowledgePointList[
              i
            ].secondKnowledgePoint;
            secondKnowledgePoint.value = this.secondKnowledgePointList[
              i
            ].questionLabelId;
      ...
      this.$axios({
        ...
        .then((res) => {
          ...
          for (var i = 0; i < this.firstKnowledgePointOptions.length; i++) {
            if (
              this.firstKnowledgePointOptions[i].label ==
              row.questionLabels[0].firstKnowledgePoint
            ) {
              this.firstKnowledgePointChoice = this.firstKnowledgePointOptions[
                i
              ].value;
            }
          }
      ...
      this.$axios({
        ...
        .then((res) => {
          ...
          for (var i = 0; i < row.questionLabels.length; i++) {
            this.eChoiceQuestionInfo.questionLabels.push(
...
.choiceQuestionShowForm {
  ...
  .choiceQuestionShowFormButtom > :first-child {
    .el-form-item__label {
      padding-right: 30px;
    }
    .el-textarea {
