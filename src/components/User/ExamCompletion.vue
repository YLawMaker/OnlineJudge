<template>
  <div class="paper-main">
    ...
    <div ref="paperContent" class="paper-content">
      <div class="subject" v-for="item in convertDatas" :key="item">

        <div class="subject-title" >
          <h2>{{item.name}}</h2><spn>（共 {{item.count}} 题，合计 {{item.totalScore}} 分）</spn>
        </div>
        
        <el-card class="box-card" v-for="(sub,index) in item.childs" :key="(sub,index)" :id="item.code+(index+1)">
            <div slot="header" class="clearfix">
            <el-tag effect="dark"> {{sub.no}} </el-tag>
            <span>{{sub.subject}}</span>
            <span>({{sub.totalScore}}分)</span>
            </div>
            <el-input :disabled="disabledAnswer" type="textarea" :rows="2"  v-model="sub.examineAnswer" resize="none" maxlength="150" @blur="answerButtionCheck($event,item,sub)"> </el-input>
        </el-card>
    ...
    <div class="paper-footer">
      <el-button type="success" @click.native="btnClick('handPaper')">交卷</el-button>
    </div>
  </div>
</template>
          ...
          list:[
            {
              ...
              type:4, no:1, subject:'以下属于南方电网员工职业操守中明文规定的有()',totalScore:6,
              answers:[
            ...
            {
              //题目类型 1.单选题 2.多选题 3.判断题 4.填空题 5.简答题
              type:4, no:1, subject:'在生产管理信息系统中，下列操作步骤能正确将工单推进流程的是（ ）',totalScore:1,
              answers:[
            ...
            {
              //题目类型 1.单选题 2.多选题 3.判断题 4.填空题 5.简答题
              type:4, no:2, subject:'在营销系统中查询客户有无欠费、余额及抄表数据接待客户时应做到哪些最基本的礼仪？',totalScore:5,
              answers:[
            ...
            {
              //题目类型 1.单选题 2.多选题 3.判断题 4.填空题 5.简答题
              type:4, no:1, subject:'记录一次与人有效沟通的案例',totalScore:10,
            ...
            {
              //题目类型 1.单选题 2.多选题 3.判断题 4.填空题 5.简答题
              type:4, no:1, subject:'请简单说一下你对礼仪的认识与理解',totalScore:10,
